  ...
  try {
    ...
    if (batch && Array.isArray(paramsArray)) {
      ...
      for (let batchStartIdx = 0; batchStartIdx < paramsArray.length; batchStartIdx += SUB_BATCH_SIZE) {
        ...
        // CRITICAL: Connect SDK BEFORE running operations in batch mode
        // This mirrors the single-mode behavior and prevents RwLock conflicts
        if (process.env.LOG_LEVEL === 'debug') {
          console.log('[Worker] Connecting SDK for sub-batch...');
        }
        await sdk.connect();
        if (process.env.LOG_LEVEL === 'debug') {
          console.log('[Worker] SDK connected for sub-batch');
        }

        // Process all items in this sub-batch with the connected SDK

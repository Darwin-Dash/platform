...
export class TestResultLogger {
    ...
    async writeResults() {
        return new Promise((resolve, reject) => {
            fs.writeFile(this.resultsFile, JSON.stringify(this.results, null, 2), (error) => {
                if (error)
                    reject(error);
                else
                    resolve();
    ...
    async updateSummary() {
        const summary = {
            ...
            averageExecutionTime: this.results.reduce((sum, r) => sum + r.totalExecutionTime, 0) / this.results.length,
        ...
        return new Promise((resolve, reject) => {
            fs.writeFile(this.summaryFile, JSON.stringify(summary, null, 2), (error) => {
                if (error)
                    reject(error);
                else
                    resolve();

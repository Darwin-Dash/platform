...
export class IdentityDiscovery {
     ...
     * ```
     */
    async discoverByHash(publicKeyHashHex) {
        ...
        try {
            ...
            const result = await runWasmOperation('identity-discover', { publicKeyHashHex }, {
            ...
            throw new Error(`Identity discovery failed: ${error.message}`);
        }
    }
    /**
     ...
     * ```
     */
    async discoverByHashBatch(publicKeyHashesHex) {
        ...
        for (let i = 0; i < publicKeyHashesHex.length; i++) {
            ...
            if (!hash || hash.length !== 40) {
                throw new Error(`Invalid public key hash at index ${i}: expected 40 hex characters, got ${hash?.length || 0}`);
            }
        }
        try {
            logger.debug(`Batch discovering ${publicKeyHashesHex.length} identities by hash (batch discovery)`);
            const { runBatchWasmOperation } = await import('../utils/wasm-worker-runner.js');
            const results = await runBatchWasmOperation('identity-discover', publicKeyHashesHex.map(hash => ({ publicKeyHashHex: hash })), {
                network: this.sdk.networkConfig.network,
                timeout: WORKER_CONFIG.BATCH_DISCOVERY_TIMEOUT_MS,
            });
            const foundCount = results.filter(r => r.found).length;
            logger.debug(`Batch discovery complete: ${foundCount} identities found out of ${publicKeyHashesHex.length}`);
            return results;
        }
        catch (error) {
            logger.error(`Batch discovery failed (${publicKeyHashesHex.length} hashes):`, error);
            throw new Error(`Batch identity discovery failed: ${error.message}`);
        }
    }
    /**
     ...
     * ```
     */
    async scanByIndex(publicKeyHashGenerator, options = {}) {
        ...
        try {
                ...
                for (let i = 0; i < batchSize && consecutiveNotFound < gapLimit; i++) {
                    ...
                    try {
                        ...
                        if (!publicKeyHash || publicKeyHash.length !== 40) {
                            throw new Error(`Invalid hash from generator at index ${index}: expected 40 hex chars, got ${publicKeyHash?.length || 0}`);
                        }
                        hashesToDiscover.push({ index, publicKeyHash });
                    }
                    catch (error) {
                        logger.error(`Failed to generate hash at index ${index}:`, error);
                        throw new Error(`Hash generation failed at index ${index}: ${error.message}`);
                    }
                }
                logger.debug(`Batch ${batchNumber}: Scanning indices ${batchStartIndex}-${batchStartIndex + hashesToDiscover.length - 1}`);
                ...
                for (let i = 0; i < results.length; i++) {
                    ...
                    if (result.found && result.identityId) {
                        ...
                        if (consecutiveNotFound >= gapLimit) {
                            logger.debug(`  Gap limit (${gapLimit}) reached at index ${item.index} - stopping scan`);
                            break;
                        }
                    }
                }
                logger.debug(`Batch ${batchNumber} complete: Found ${foundInBatch} identities ` +
            ...
            logger.info(`Scan complete: ${foundIdentities.length} identities found ` +
                `across ${batchNumber} batches`);
            return foundIdentities;
        }
        catch (error) {
            logger.error(`Identity scan failed:`, error);
            throw new Error(`Identity scan failed: ${error.message}`);
        }
    }
}

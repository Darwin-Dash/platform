...
export class CreditOperations {
    ...
    async creditTransfer(args) {
        ...
        try {
            logger.info(`Transferring ${numAmount} credits from ${senderId} to ${recipientId}` +
                (keyId !== undefined ? ` (keyId: ${keyId})` : ''));
            const w = await this.sdk.getWasmSdkConnected();
            const result = await w.identityCreditTransfer(senderId, recipientId, BigInt(numAmount), privateKeyWif, keyId ?? null);
    ...
    async creditWithdrawal(args) {
        ...
        if (typeof coreFeePerByte !== 'number' || coreFeePerByte < 0) {
            throw new Error(`Invalid coreFeePerByte: ${coreFeePerByte}. Must be a non-negative number`);
        }
        try {
            logger.info(`Withdrawing ${numAmount} credits from ${identityId} to ${toAddress}` +
                (keyId !== undefined ? ` (keyId: ${keyId})` : ''));
            const w = await this.sdk.getWasmSdkConnected();
            const result = await w.identityCreditWithdrawal(identityId, toAddress, BigInt(numAmount), coreFeePerByte ?? null, privateKeyWif, keyId ?? null);
            ...
            throw new Error(`Credit withdrawal failed: ${error.message}`);
        }
    }
    /**
    ...
    validateAmount(amount, operation) {
        ...
        if (!Number.isInteger(numAmount) || numAmount < 0) {
            throw new Error(`Invalid amount for ${operation}: ${numAmount}. Must be a non-negative integer`);
        }
        // Check reasonable bounds (0 to 100 billion duffs)
        const MAX_AMOUNT = 100_000_000_000; // 100 billion duffs = 1 million DASH
        if (numAmount > MAX_AMOUNT) {
            throw new Error(`Amount exceeds maximum: ${numAmount} > ${MAX_AMOUNT}. Maximum is 100 billion duffs`);
        }
        return numAmount;
    }
}

...
export class IdentityUpdater {
     ...
     * ```
     */
    async topUpWithWallet(identityId, amount, mnemonic, options = {}) {
        ...
        try {
            ...
            // Emit start event
            if (onProgress) {
                onProgress(OperationEventFactory.phaseStart('wallet_setup', 'Initializing wallet and discovering UTXOs...'));
            }
            // Step 1: Setup wallet
            ...
            if (!walletSetup.latestUTXO) {
                throw new WalletSetupError('utxo_discovery', 'No UTXOs available - cannot top up identity', false, { availableUTXOs: walletSetup.utxos.length });
            }
            ...
            if (onProgress) {
                onProgress(OperationEventFactory.phaseStart('identity_topup', 'Submitting top-up to Platform...'));
            }
            ...
            if (onProgress) {
                onProgress(OperationEventFactory.phaseComplete('finalization', 'Top-up completed successfully'));
            }
            ...
            if (error instanceof Error) {
                throw new PlatformSubmissionError('identity_topup', `Top-up failed: ${error.message}`, identityId, false, { originalError: error.message });
            }
     ...
     * @returns Top-up result
     */
    async topUpWithAccount(identityId, account, amount, options = {}) {
        logger.warn('topUpWithAccount() is deprecated - use topUpWithWallet() instead for cleaner architecture');
        if (!identityId) {
            throw new ValidationError('identityId_validation', 'Identity ID is required', 'identityId', false);
        }
        if (!account) {
        ...
        if (typeof amount !== 'number' || isNaN(amount) ||
            ...
            throw new ValidationError('amount_validation', `Invalid amount: must be between ${IDENTITY_CONFIG.TOPUP_MIN_AMOUNT} and ${IDENTITY_CONFIG.MAX_AMOUNT}`, 'amount', false, { amount });
        }
        try {
            ...
            const proofManager = new AssetLockProofManager(this.sdk);
            const transactionData = await proofManager.waitForConfirmation(undefined, transactionIdString, txResult.transactionHex, [transactionOptions.change]);
            // Submit to Platform
            const { runWasmOperation } = await import('../utils/wasm-worker-runner.js');
            const result = await runWasmOperation('identity-topup', {
                identityId,
            ...
            throw new PlatformSubmissionError('identity_topup', `Top-up failed: ${error.message}`, identityId, false);
        }
    }
    /**

 ...
 */
async function runNodeWorker(operation, params, options) {
 ...
 */
async function runWebWorker(operation, params, options) {
    ...
    return new Promise((resolve, reject) => {
        ...
        try {
            ...
            reject(new Error(`Failed to create Web Worker: ${error instanceof Error ? error.message : String(error)}. ` +
        ...
        worker.onerror = (error) => {
            clearTimeout(timeoutHandle);
            worker.terminate();
            reject(new Error(`Worker process error: ${error.message || error.error?.message || 'Unknown error'}`));
        };
        // Send operation request to worker
        const message = {
            operation,
 ...
 */
export async function runWasmOperation(operation, params, options = {}) {
    if (isNodeEnvironment()) {
        // Node.js: Use child_process fork
        return runNodeWorker(operation, params, options);
    }
    else {
        // Browser: Use Web Worker (Phase 2)
        return runWebWorker(operation, params, options);
    }
}
/**
 ...
 */
export async function runBatchWasmOperation(operation, paramsArray, options = {}) {

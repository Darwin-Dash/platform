...
export class IdentityKeyGenerator {
     ...
     */
    async generateFromWallet(freshAccount, identityIndex) {
        ...
        for (const config of IdentityKeyGenerator.KEY_CONFIGS) {
            try {
                ...
                throw new Error(`Failed to generate identity key ${config.id}: ${error.message}`);
            }
        }
     ...
     */
    validatePublicKey(publicKeyHex, keyId) {
        // Validate hex format

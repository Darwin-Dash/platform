...
import { validateIdentityIndex } from './utils/identity-transformer.js';
...
export class StateManager {
  ...
  // Get identities that need re-discovery (missing/invalid index)
  getIdentitiesNeedingReDiscovery() {
    const needsReDiscovery = [];
    for (const [id, identity] of this.state.identities) {
      if (identity.needsReDiscovery === true) {
        needsReDiscovery.push(identity);
      }
    }
    return needsReDiscovery;
  }

  // Clear the needsReDiscovery flag after successful re-discovery
  clearReDiscoveryFlag(identityId) {
    const identity = this.state.identities.get(identityId);
    if (identity && identity.needsReDiscovery) {
      delete identity.needsReDiscovery;
  ...
  restore() {
    try {
      ...
      console.log(`  Restoring state: ${parsed.identities?.length || 0} identities, ${parsed.transactions?.length || 0} transactions`);

      // Restore identities
      if (parsed.identities) {
        this.state.identities = new Map(parsed.identities);

        // Validate each identity has a valid index for key derivation
        let invalidCount = 0;
        for (const [id, identity] of this.state.identities) {
          const validation = validateIdentityIndex(identity);
          if (!validation.valid) {
            invalidCount++;
            console.warn(`Restored identity ${id.substring(0, 12)}... has invalid index (${validation.reason}) - marking for re-discovery`);
            identity.needsReDiscovery = true;
          }
        }
        if (invalidCount > 0) {
          console.warn(`${invalidCount} identity/identities need re-discovery due to missing/invalid index`);
        }
      }
...
// Persist on page unload to prevent data loss (safety net) - browser only

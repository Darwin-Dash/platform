 ...
 * NOTE: This function expects CREDITS as input.
 * The SDK returns identity.balance in CREDITS (confirmed via Platform Explorer).
 *
 * Conversion: 1 duff = 1000 credits
 * 1 DASH = 100,000,000 duffs = 100,000,000,000 credits
 * @param {number} credits - Amount in credits (as returned by SDK)
 * @returns {object} Object with credits, duffs, and dash formatted strings
 */
 ...
 * @param {string|number} purpose - Key purpose (string or numeric enum)
 * @returns {string} Formatted purpose
 */
export function formatKeyPurpose(purpose) {
  // Handle numeric enum values from WASM SDK
  const numericPurposeMap = {
    0: 'Authentication',
    1: 'Encryption',
    2: 'Decryption',
    3: 'Transfer',
  ...
  if (typeof purpose === 'number') {
    return numericPurposeMap[purpose] || `Purpose ${purpose}`;
  }

  return stringPurposeMap[purpose] || String(purpose);
}

/**
 * Format security level for display
 * @param {string|number} level - Security level (string or numeric enum)
 * @returns {object} Level text and CSS class
 */
export function formatSecurityLevel(level) {
  // Handle numeric enum values from WASM SDK
  const numericLevelMap = {
    0: { text: 'Master', class: 'level-master' },
    1: { text: 'Critical', class: 'level-critical' },
    2: { text: 'High', class: 'level-high' },
    3: { text: 'Medium', class: 'level-medium' }
  };
  ...
  if (typeof level === 'number') {
    return numericLevelMap[level] || { text: `Level ${level}`, class: 'level-unknown' };
  }

  return stringLevelMap[level] || { text: String(level), class: 'level-unknown' };
}

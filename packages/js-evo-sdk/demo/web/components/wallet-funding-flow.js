...
export class WalletFundingFlow {
        ...
        <div class="funding-help">
          <button class="btn btn-link" id="need-help-btn">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" style="margin-right: var(--space-2);">
              <path d="M9 18c0 2 2 3 3 3s3-1 3-3" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>
              <path d="M12 2a7 7 0 017 7c0 2.38-1.19 4.47-3 5.74V17a2 2 0 01-2 2h-4a2 2 0 01-2-2v-2.26C6.19 13.47 5 11.38 5 9a7 7 0 017-7z" stroke="currentColor" stroke-width="2"/>
            </svg>
  ...
  async showTimeframeStep() {
    ...
    // Generate funding address if not already set
    ...
    container.innerHTML = `
      <div class="funding-step funding-timeframe">
        ...
        <div class="funding-address-reminder">
          <div class="address-reminder-header">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" style="color: var(--color-warning);">
              <circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="2"/>
              <path d="M12 8v4M12 16h.01" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>
            </svg>
            <span>Funds should have been sent to:</span>
          </div>
          <div class="address-display-compact">
            <code class="funding-address-text">${this.fundingAddress}</code>
            <button class="btn btn-icon btn-sm copy-address-btn" data-copy="${this.fundingAddress}" title="Copy address">
          ...
          <div class="progress-stages">
            <div class="stage stage-tx" id="stage-tx">
              <span class="stage-icon">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" class="icon-pending">
  ...
  _updateStage(stageId, status) {
    ...
    if (iconEl) {
      if (status === 'complete') {
        iconEl.innerHTML = `
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" class="icon-complete">
            <circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="2"/>
            <path d="M9 12l2 2 4-4" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
          </svg>`;
      } else {
  ...
  _updateScanProgressUI(progress) {
    ...
    const syncedBlocks = progress.syncedBlocks || 0;
    const totalBlocks = progress.totalBlocks || 1; // Prevent division by zero
    const percent = progress.progress ?? Math.round((syncedBlocks / totalBlocks) * 100);

    if (fillEl) fillEl.style.width = `${Math.min(percent, 100)}%`;
    if (scannedEl) scannedEl.textContent = syncedBlocks.toLocaleString();
    if (totalEl) totalEl.textContent = totalBlocks.toLocaleString();
      ...
      <div class="funding-step funding-confirmed">
        ...
        ${validation.warning ? `
          <div class="funding-warning">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" style="color: var(--color-warning); flex-shrink: 0;">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <div class="help-section">
            <h3>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" style="vertical-align: text-bottom; margin-right: 4px;">
                <path d="M12 2v6m0 0l-3-3m3 3l3-3" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                <path d="M6 12h12v8a2 2 0 01-2 2H8a2 2 0 01-2-2v-8z" stroke="currentColor" stroke-width="2"/>
                <path d="M9 12v4m6-4v4" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>
              </svg>
              Testnet Faucets
            </h3>
          ...
          <div class="help-section">
            <h3>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" style="vertical-align: text-bottom; margin-right: 4px;">
                <circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="2"/>
                <path d="M9 9a3 3 0 115.12 2.12c-.68.68-1.62 1.38-2.12 2.38V15" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>
  ...
  async checkExistingBalance(timeframe) {
    ...
    // Estimate block count based on timeframe (~1 block per 2.5 minutes)
    ...
    const totalEstimate = estimatedBlocks[timeframe] || 100;
      ...
      <div class="funding-step funding-scanning">
        ...
        <div class="scan-progress-container">
          ...
          <p class="scan-progress-text">
            <span id="scan-blocks-scanned">0</span> / <span id="scan-blocks-total">~${totalEstimate}</span> blocks
    ...
    container.innerHTML = `
      <div class="funding-step no-funds-found">
        <div class="no-funds-icon">
          <svg width="48" height="48" viewBox="0 0 24 24" fill="none" style="color: var(--color-gray-400);">
        ...
        <div class="funding-address-reminder" style="margin-bottom: var(--space-4);">
          <div class="address-reminder-header">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" style="color: var(--color-info);">
              <circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="2"/>
              <path d="M12 8v4M12 16h.01" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>
            </svg>
            <span>Funds should have been sent to this address:</span>
          </div>
          <div class="address-display-compact">
            <code class="funding-address-text">${this.fundingAddress}</code>
            <button class="btn btn-icon btn-sm copy-address-btn" data-copy="${this.fundingAddress}" title="Copy address">
        ...
        <div class="no-funds-options">
          ...
          <ul>
            ...
            <li>Verify the address above matches where you sent funds</li>
          </ul>
        </div>
        <div class="funding-actions">

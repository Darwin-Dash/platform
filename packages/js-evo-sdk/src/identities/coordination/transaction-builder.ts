...
export interface AssetLockTransactionOptions {
  amount: number;
  utxo?: UTXO;                           // Optional for backward compat with deprecated pattern
  sourceAddress?: DerivedAddressInfo;    // Optional for backward compat with deprecated pattern
  changeAddress: string;
  network?: string;
  // Deprecated pattern support for old methods
  account?: any;
  utxos?: UTXO[];
}

export interface AssetLockTransactionResult {
...
export class TransactionBuilder {
   ...
   */
  async createAssetLockTransaction(
    options: AssetLockTransactionOptions
    ...
    if (logger.isDebugEnabled()) {
      ...
      logger.debug(`   Output 0 script length: ${(payload.creditOutputs[0].script as any).toString().length}`);
    }
    ...
    try {
      ...
      // @ts-ignore - dashcore-lib API type incompatibilities
      const script = new Script().add((Opcode as any).OP_RETURN).add((Buffer as any).alloc(0));

      transaction
        .setType(8) // Asset lock type
        .from([unspentOutput])
        .addOutput(
        ...
        // @ts-ignore - dashcore-lib payload type mismatch
   ...
   *
   * Uses DAPIClient directly (no wallet-lib account needed).
  ...
  async broadcastTransaction(
    ...
    // Broadcast using DAPIClient (automatic retry + failover)
    const transactionId = await dapiClient.core.broadcastTransaction(

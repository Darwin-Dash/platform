...
export class WalletCoordinator {
  ...
  async setupWallet(options: WalletSetupOptions): Promise<WalletSetupResult> {
    ...
    // Step 2: Create DAPIClient with resilient configuration
    logger.debug('🌐 Creating DAPIClient...');

    // Use explicit addresses if provided - bypasses slow/failing seed node resolution
    // When DAPI_ADDRESSES is set, we directly connect to those nodes
    // Otherwise fall back to seed discovery (can be slow on testnet)
    let dapiAddresses: string[] | undefined;
    if (process.env.DAPI_ADDRESSES) {
      dapiAddresses = process.env.DAPI_ADDRESSES.split(',').map(a => a.trim());
      logger.debug(`   Using explicit DAPI addresses: ${dapiAddresses.join(', ')}`);
    }

    const dapiClientOptions: any = {
      ...
      // Pass explicit addresses if available - this speeds up connection significantly
  ...
  async getCurrentBlockHeight(): Promise<number> {
    ...
    // Use explicit DAPI addresses if provided for faster connection
    if (process.env.DAPI_ADDRESSES) {

...
The UTXO Finder requires a DAPI client for blockchain communication. We **strongly recommend** using `ResilientDAPIClient` for production applications, which provides automatic retry, node failover, and graceful degradation.

### Recommended: ResilientDAPIClient (Production)

```javascript
const { UTXOFinder } = require('@dashevo/dash-utxo-finder');
const { ResilientDAPIClient } = require('@dashevo/resilient-dapi-client');

// Create resilient client with automatic retry and failover
const dapiClient = new ResilientDAPIClient({
  ...
  retries: 5,
  logLevel: 'info',
  enableObservability: true,      // Event monitoring
  enableGracefulDegradation: true, // Platform failures don't break core
  enableAdaptiveRetry: true        // Exponential backoff
});

// Monitor reliability
dapiClient.on('retry', (event) => {
  console.log(`Retry attempt ${event.attempt} for ${event.method}`);
});

dapiClient.on('failover', (event) => {
  console.log(`Failed over from ${event.oldNode} to ${event.newNode}`);
});
...
- ✅ **Node Failover**: Switches to healthy nodes automatically
- ✅ **Graceful Degradation**: Core blockchain ops continue even if platform fails
- ✅ **Observability**: Track failures and failovers via events
- ✅ **Production-Ready**: Battle-tested resilience patterns

### Alternative: Legacy DAPIClient

For simple use cases or testing, you can use the basic `DAPIClient`:

```javascript
const DAPIClient = require('@dashevo/dapi-client');

// Initialize DAPI client (no retry or failover)
...
The UTXO finder uses a lightweight, environment-variable-based logging system that respects the `LOG_LEVEL` environment variable. This provides production-ready logging control without requiring additional configuration.

### Log Levels

The library supports the following log levels (from most to least verbose):

- **`TRACE`** - Most verbose, internal operations (rarely needed)
- **`DEBUG`** - Detailed debugging information (transaction hashes, merkle blocks, stream details)
- **`INFO`** - General operational information (header sync progress, sync summaries) - **Default**
- **`WARN`** - Warnings about recoverable issues (invalid addresses, failed parsing)
- **`ERROR`** - Critical errors that affect functionality
- **`SILENT`** - No logging output

### Setting Log Level

Control logging output by setting the `LOG_LEVEL` environment variable before running your application:

```bash
# Development: verbose debugging
LOG_LEVEL=debug node your-app.js

# Production: informational messages only (default)
LOG_LEVEL=info node your-app.js

# Production: errors only
LOG_LEVEL=error node your-app.js

# Testing: no log output
LOG_LEVEL=silent npm test
```

### Log Contexts

The library uses context-prefixed logging to help identify the source of log messages:

- `[TransactionSyncer]` - Transaction stream processing and merkle block handling
- `[BloomFilterBuilder]` - Bloom filter creation and address validation
- `[LocalStorageAdapter]` - Browser localStorage operations
- `[StreamWrapper]` - gRPC stream conversion and handling
- `[UTXOFinder]` - High-level UTXO discovery orchestration

### Examples

**Development (DEBUG level):**
```bash
LOG_LEVEL=debug node scripts/find-utxo.js
```
Output:
```
[INFO] [TransactionSyncer] Pre-syncing headers from 1000000 to 1000100 via streaming API
[INFO] [TransactionSyncer] Cached 50/100 headers
[INFO] [TransactionSyncer] Header pre-sync complete: 100 headers cached
[DEBUG] [TransactionSyncer] Bloom filter details: { hasBloomFilter: true, vDataLength: 500 ... }
[DEBUG] [TransactionSyncer] Processing 3 raw transactions
[DEBUG] [TransactionSyncer] Found TX: a1b2c3d4...
[INFO] [TransactionSyncer] Sync complete - Summary: { totalTransactions: 3, ... }
```
...
```
Output: (only errors are logged, all INFO/DEBUG messages suppressed)

**Testing (SILENT):**
```bash
LOG_LEVEL=silent npm test
```
Output: (no logging, clean test output)

### Performance Considerations

The logging system includes conditional checks for expensive debug operations:

```javascript
// Debug logs with expensive object serialization are only evaluated when debug is enabled
if (this.logger.isDebugEnabled()) {
  this.logger.debug('Processing merkle block:', { /* expensive serialization */ });
}
```

This ensures that debug-level performance overhead is eliminated in production when using `INFO`, `WARN`, or `ERROR` log levels.
...
**Parameters:**
- `dapiClient`: ResilientDAPIClient (recommended) or legacy DAPIClient instance
- `network`: 'mainnet', 'testnet', or 'regtest' (default: 'testnet')

The constructor accepts any client implementing the `DAPIClientLike` interface, which includes both `ResilientDAPIClient` and the legacy `DAPIClient`.

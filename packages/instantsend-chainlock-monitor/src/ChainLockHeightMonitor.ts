...
export class ChainLockHeightMonitor extends EventEmitter {
  ...
  constructor(
    ...
    // Provide no-op logger if not provided
    this.logger = logger || {
      debug: () => {},
      info: () => {},
      warn: () => {},
  ...
  private async poll(): Promise<void> {
    try {
      ...
      const response = await this.dapiClient.platform.getEpochsInfo(0, 1, { prove: false });
      ...
      if (coreChainLockedHeight > this.lastChainLockedHeight) {
        ...
        const confirmedTxids = this.tracker.recordChainLock(coreChainLockedHeight, timestamp, this.logger);

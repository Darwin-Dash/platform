...
import { TrackedTransaction, TransactionStatus } from './types.js';
import { Logger } from './utils/logger.js';

export class TransactionTracker {
  private transactions: Map<string, TrackedTransaction>;
  private blockHeightMap: Map<number, string[]>;
  private monitoredTxIds: Set<string>;
  private autoPrune: boolean;
  private maxTransactions: number;

  constructor(autoPrune: boolean = false, maxTransactions: number = 1000) {
   ...
   */
  recordChainLock(chainLockedHeight: number, timestamp: number, logger?: Logger): string[] {
    const confirmedTxids: string[] = [];

    if (logger) {
      logger.debug(`🔍 recordChainLock(${chainLockedHeight}), blockHeightMap.size=${this.blockHeightMap.size}`);
    }
    ...
    for (const [blockHeight, txids] of this.blockHeightMap.entries()) {
      if (logger) {
        logger.debug(`   Checking block ${blockHeight}: ${txids.length} txids`);
      }
    ...
    if (logger) {
      logger.debug(`   Result: ${confirmedTxids.length} confirmed`);
    }

    // Auto-prune if enabled (opt-in for long-running services)
    if (this.autoPrune && confirmedTxids.length > 0) {
      if (logger) {
        logger.debug(`   Auto-pruning ${confirmedTxids.length} confirmed transactions`);
      }

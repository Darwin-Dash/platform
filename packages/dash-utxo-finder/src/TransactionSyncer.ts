...
import { makeAsyncIterable } from './StreamWrapper';
...
export class TransactionSyncer {
  ...
  constructor(dapiClient: any, network: string = 'testnet') {
  ...
  /**
   * Get core client (handle both async and sync access patterns)
   * @private
   */
  private async getCore(): Promise<any> {
    // Handle both synchronous property access (real DAPI client) and async method access (mocks)
    if (typeof this.dapiClient.core === 'function') {
      // core is an async method (like in mocks) - call with proper context binding
      return await this.dapiClient.core();
    }
    // core is already a property/object (like in real DAPI client)
  ...
  private async syncHeaders(
    ...
    console.log(`[TransactionSyncer] Pre-syncing headers from ${fromHeight} to ${toHeight} via streaming API`);
        ...
        if (blockHeaders) {
          ...
          headersList.forEach((headerBuf: any) => {
            try {
              ...
              console.warn(`[TransactionSyncer] Failed to parse header:`, (error as Error).message);
            }
          });

          console.log(`[TransactionSyncer] Cached ${syncedHeaders}/${totalHeaders} headers`);
        }
      }

      console.log(`[TransactionSyncer] Header pre-sync complete: ${this.headerCache.size} headers cached`);
    } catch (error) {
      console.error('[TransactionSyncer] Error during header pre-sync:', error);
  ...
  private async _performSync(
    ...
    if (!toHeight) {
      try {
        ...
        console.warn('Using fromHeight as fallback for toHeight');
        toHeight = validatedFromHeight;
      }
    }
    ...
    try {
      ...
      // Get the core client (handles both real client and mocks)
      const core = await this.getCore();
      ...
      // Log bloom filter details for diagnostics
      console.log('[TransactionSyncer] Bloom filter details:', {
      ...
      console.log('[TransactionSyncer] Calling subscribeToTransactionsWithProofs:', {
      ...
      for await (const message of stream) {
        ...
        // Check each getter individually as not all may exist
        ...
        console.log(`[TransactionSyncer] Message ${messageCount}:`, {
        ...
        // Process raw transactions from the message
        // Handle both protobuf (with getTransactionsList) and plain array formats
        if (rawTxs) {
          ...
          if (txList && txList.length > 0) {
            ...
            console.log(`[TransactionSyncer] Processing ${txs.length} raw transactions`);
            txs.forEach((tx: any) => {
              console.log(`[TransactionSyncer] Found TX: ${tx.hash}`);
              transactions.push({
                tx,
        ...
        // Process merkle block (provides block metadata)
        if (rawMerkle) {
          merkleBlockCount++;
          try {
            ...
            console.log(`[TransactionSyncer] Processing merkle block ${merkleBlockCount}:`, {
              blockHash,
            ...
            if (!cachedHeader) {
              console.warn(`[TransactionSyncer] WARNING: Block ${blockHash} not in header cache!`);
            }
            ...
            const txHashesInBlock = new Set(
              merkleBlock.hashes.map((h: any) => {
                ...
                console.log(`[TransactionSyncer] Merkle block TX hash: ${hashStr} -> reversed: ${reversed}`);
                return reversed;
              })
            );

            console.log(`[TransactionSyncer] Attempting to match ${txHashesInBlock.size} merkle hashes with ${transactions.length} transactions`);
            ...
            transactions
              .filter(({ tx }) => {
                ...
                if (match) {
                  console.log(`[TransactionSyncer] ✓ Matched TX ${tx.hash} to merkle block`);
                  matchedCount++;
                } else {
                  console.log(`[TransactionSyncer] ✗ TX ${tx.hash} not in merkle block`);
                }
                return match;
              })
              .forEach((txData) => {
            ...
            console.log(`[TransactionSyncer] Matched ${matchedCount} transactions to merkle block`);
            ...
            if (onProgress) {
              onProgress({
                ...
                syncedBlocks,
                totalBlocks,
                currentHeight: validatedFromHeight + syncedBlocks,
              });
            }
          } catch (error) {
            console.warn('[TransactionSyncer] Failed to process merkle block:', error);
          }
        }

        // Process instant lock messages (if available)
        // InstantSend locks provide instant confirmation before blockchain confirmation
        ...
        // Process chainlock messages (if available)
        // ChainLocks provide finality for entire blocks via LLMQ consensus
      ...
      console.log(`[TransactionSyncer] Sync complete - Summary:`, {
      ...
      // Log each transaction's final metadata state
      transactions.forEach(({ tx, metadata }, index) => {
        console.log(`[TransactionSyncer] TX ${index + 1}/${transactions.length}:`, {
          hash: tx.hash,
      ...
      console.error('[TransactionSyncer] Error during transaction sync:', error);
  ...
  private async getBlockHeight(blockHash: string): Promise<number> {
    console.log(`[TransactionSyncer] getBlockHeight called for hash: ${blockHash}`);
    try {
      // Query DAPI for block info by hash
      const core = await this.getCore();
      console.log(`[TransactionSyncer] Calling core.getBlockByHash(${blockHash})`);
      const blockInfo = await core.getBlockByHash(blockHash);
      console.log(`[TransactionSyncer] getBlockByHash response:`, blockInfo);
      const height = blockInfo.height || 0;
      console.log(`[TransactionSyncer] Extracted height: ${height}`);
      return height;
    } catch (error) {
      console.warn(`[TransactionSyncer] Failed to get block height for hash ${blockHash}:`, error);
      return 0;
    }
  }
}

...
export class TransactionBroadcaster {
  constructor(private client: DashRpcClient) {}
  ...
  async sendToAddress(address: string): Promise<BroadcastResult> {
    ...
    if (outputAmount <= 0) {
      throw new Error(
        `Insufficient balance: ${totalInput} DASH (need > ${fee} DASH for fee)`
      );
    }
  ...
  async consolidateUTXOs(
    ...
    if (outputAmount <= 0) {
      throw new Error(
        `Insufficient funds: total input ${totalInput} DASH, fee ${fee} DASH`
      );
    }

...
export class BloomFilterBuilder {
     ...
     * ```
     */
    static build(addresses, network, falsePositiveRate = this.BLOOM_FALSE_POSITIVE_RATE) {
        // Handle empty address array - return minimal valid bloom filter
        if (addresses.length === 0) {
            logger.warn('Building bloom filter with zero addresses');
            const emptyFilter = BF.create(1, // nElements - minimum size
            falsePositiveRate, 0, // nTweak
            BF.BLOOM_UPDATE_ALL);
        ...
        const filter = BF.create(addresses.length * 10, // nElements
        falsePositiveRate, 0, // nTweak
        BF.BLOOM_UPDATE_ALL);
     ...
     */
    static buildForAddress(address, network, falsePositiveRate = this.BLOOM_FALSE_POSITIVE_RATE) {
     ...
     * @returns Bloom filter as plain object with vData, nHashFuncs, nTweak, nFlags
     */
    static buildFromHashes(addressHashes, falsePositiveRate = this.BLOOM_FALSE_POSITIVE_RATE) {
        // Handle empty hash array
        if (addressHashes.length === 0) {
            logger.warn('Building bloom filter with zero address hashes');
            const emptyFilter = BF.create(1, // nElements - minimum size
            falsePositiveRate, 0, BF.BLOOM_UPDATE_ALL);
            return this.filterToParams(emptyFilter);
        }
        const filter = BF.create(addressHashes.length * 10, falsePositiveRate, 0, BF.BLOOM_UPDATE_ALL);
...
BloomFilterBuilder.BLOOM_FALSE_POSITIVE_RATE = DEFAULT_BLOOM_FALSE_POSITIVE_RATE;
//# sourceMappingURL=BloomFilterBuilder.js.map
...
export class TransactionSyncer {
    constructor(dapiClient, network = 'testnet') {
     ...
     * @private
     */
    validateBlockHeight(height, fieldName) {
        ...
        if (!Number.isFinite(heightNum)) {
            throw new Error(`${fieldName} must be a valid number, got: ${height} (type: ${typeof height})`);
        }
        ...
        if (heightNum > TransactionSyncer.MAX_UINT32) {
            throw new Error(`${fieldName} exceeds maximum value (${TransactionSyncer.MAX_UINT32}), got: ${heightNum}`);
        }
        // Return as integer
     ...
     * @private
     */
    async retryOnNotFound(operation, operationName, maxRetries = 5) {
        let lastError = null;
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                if (isNotFound && attempt < maxRetries) {
                    ...
                    this.logger.warn(`${operationName} failed with NOT_FOUND (attempt ${attempt}/${maxRetries})` +
    ...
    banLastUsedNode() {
        try {
            ...
            const transport = core.grpcTransport;
     ...
     */
    async syncTransactions(bloomFilter, fromHeight, toHeight, onProgress, timeout) {
        ...
        if (this.syncInProgress) {
            throw new Error('Transaction sync already in progress. Wait for current sync to complete or create a new TransactionSyncer instance.');
        }
                ...
                if (blockHeaders) {
                    ...
                    headersList.forEach((headerBuf) => {
                        try {
                            ...
                            const BlockHeader = dashcore.BlockHeader;
                            ...
                            this.logger.warn(`Failed to parse header:`, error.message);
                        }
                    });
     ...
     * @private
     */
    async validateHeightParameters(fromHeight, toHeight) {
        // Validate fromHeight first
        let validatedFromHeight;
        try {
            ...
            try {
                ...
                toHeight =
                    status?.chain?.blocksCount ||
                        ...
                        undefined;
                if (!toHeight) {
                    this.logger.warn('Could not determine chain height from blockchain status, using fromHeight as fallback');
                    toHeight = validatedFromHeight;
                }
            }
            catch (error) {
                this.logger.warn(`Failed to get chain status: ${error instanceof Error ? error.message : String(error)}`);
        ...
        if (validatedFromHeight < 1) {
            this.logger.warn(`fromHeight (${validatedFromHeight}) is less than 1 (genesis non-spendable), setting to 1`);
            validatedFromHeight = 1;
        }
        // Ensure toHeight >= fromHeight
        if (validatedToHeight < validatedFromHeight) {
            this.logger.warn(`toHeight (${validatedToHeight}) is less than fromHeight (${validatedFromHeight}), swapping values`);
     ...
     * @private
     */
    extractRawTransactions(msg, transactions) {
     ...
     * @private
     */
    processMerkleBlock(msg, transactions, validatedFromHeight, syncedBlocks, totalBlocks, onProgress) {
        ...
        if (rawMerkle) {
            ...
            try {
                ...
                const txHashesInBlock = new Set(merkleBlock.hashes.map((h) => {
                ...
                transactions
                    ...
                    txData.metadata = {
                        ...
                        status: 'pending', // Will be updated by lock messages
                    };
                });
                if (this.logger.isDebugEnabled()) {
     ...
     * @private
     */
    processInstantLocks(msg, transactions) {
        // Try both accessor patterns (protobuf getters vs plain properties)
        ...
        if (instantLocks && Array.isArray(instantLocks)) {
            instantLocks.forEach((lockMsg) => {
                try {
                    ...
                    const txData = transactions.find(({ tx }) => tx.hash === instantLock.txid);
     ...
     * @private
     */
    processChainLocks(msg, transactions) {
        // Try both accessor patterns (protobuf getters vs plain properties)
        ...
        if (chainLocks && Array.isArray(chainLocks)) {
            chainLocks.forEach((lockMsg) => {
                try {
                    ...
                    transactions
                        .filter(({ metadata }) => metadata && metadata.blockHash === chainLock.blockHash)
                        .forEach((txData) => {
                        if (txData.metadata) {
     ...
     * @private
     */
    async _performSync(bloomFilter, fromHeight, toHeight, onProgress, timeout) {
        // Validate and normalize height parameters
        const { validatedFromHeight, validatedToHeight } = await this.validateHeightParameters(fromHeight, toHeight);
        const transactions = [];
        ...
        try {
            ...
            let rawStream = await this.retryOnNotFound(async () => {
                const s = core.subscribeToTransactionsWithProofs(bloomFilter, {
                    fromBlockHeight: validatedFromHeight,
            ...
            for await (const message of stream) {
                ...
                const merkleResult = this.processMerkleBlock(msg, transactions, validatedFromHeight, syncedBlocks, totalBlocks, onProgress);
     ...
     * @private
     */
    parseInstantSendLock(isdlockBuffer) {
        try {
            // Use dashcore-lib InstantLock class to parse buffer
            const instantLock = new InstantLock(isdlockBuffer);
            return {
            ...
            throw new Error(`Failed to parse InstantSend lock message: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
     ...
     * @private
     */
    parseChainLock(chainlockBuffer) {
        try {
            // Use dashcore-lib ChainLock class to parse buffer
            const chainLock = new ChainLock(chainlockBuffer);
            return {
                height: chainLock.height,
                blockHash: typeof chainLock.blockHash === 'string'
                    ? chainLock.blockHash
                    : chainLock.blockHash.toString('hex'),
                signature: typeof chainLock.signature === 'string'
                    ? chainLock.signature
                    : chainLock.signature.toString('hex'),
            };
        }
        catch (error) {
            throw new Error(`Failed to parse ChainLock message: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
...
TransactionSyncer.MAX_UINT32 = 4294967295;
//# sourceMappingURL=TransactionSyncer.js.map